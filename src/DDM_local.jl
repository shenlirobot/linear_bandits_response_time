# Copied from https://raw.githubusercontent.com/itsdfish/SequentialSamplingModels.jl/master/src/type_system.jl
...
# Copied from https://github.com/itsdfish/SequentialSamplingModels.jl/blob/master/src/DDM.jl
"""
    DDM{T<:Real} <: SSM2D
...
"""
# Shen Li: This conflicts with other rand functions. So we comment it out.
...
function simulate(rng::AbstractRNG, model::DDM; Δt=0.001)
    ...
    while (x < α) && (x > 0)
        t += Δt
        x += ν * Δt + Base.rand(rng, Normal(0.0, 1.0)) * √(Δt)
        push!(evidence, x)
        push!(time_steps, t)
    end
...
# Shen Li: a helper function
function simulate_choice_rt(rng::AbstractRNG, model::DDM; Δt=0.001)
    (time_steps, evidence) = simulate(rng, model; Δt=Δt)
    if evidence[end] <= 0
        choice = 2
    elseif evidence[end] >= model.α
        choice = 1
    else
        error("Impossible")
    end
    rt = time_steps[end] + model.τ
    return choice, rt
end